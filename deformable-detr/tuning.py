...
def get_args_parser():
    ...
    parser.add_argument(
        '--coco_path', default='/home/ayina/MscThesis/DCW/datasets/Dataset_final/DATA_0_COCO_format/', type=str)
    ...
    parser.add_argument(
        '--resume', default='./weights/r50_deformable_detr_plus_iterative_bbox_refinement_plus_plus_two_stage-checkpoint.pth', help='resume from checkpoint')
...
def update_args_(args, params):  # change args in-place
    uargs = vars(args)
    uargs.update(params)
...
def main(trials=None):
    ...
    if trials is not None:
        params = {'lr': trials.suggest_float('lr', 2e-5, 3e-4, step=1e-5),
                  'lr_backbone': trials.suggest_float('lr_backbone', 2e-6, 3e-5, step=1e-6),
