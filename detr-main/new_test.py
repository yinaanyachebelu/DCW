...
@torch.no_grad()
def infer(images_path, model, postprocessors, device):
    model.eval()
    duration = 0
    ...
    for img_sample in images_path:
        ...
        image_id = int(Path(img_sample).stem) if Path(img_sample).stem.isnumeric() else Path(img_sample).stem
...
if __name__ == "__main__":
    ...
    infer(image_paths, model, postprocessors, device)
